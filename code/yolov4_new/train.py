...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Freeze
    if len(freeze) > 1:
        freeze = [ 'model.%d.' % x for x in freeze ]
    else:
        freeze = [ 'model.%d.' % i for i in range(freeze[0]) ]  # layers to freeze
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                torch.save({'state_dict':ckpt['model'].state_dict()}, last)
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = validate.run(
                        ...
                        batch_size=batch_size // WORLD_SIZE * 2,
                        imgsz=imgsz,
                        model=attempt_load(f, torch.device('cpu')).half(),
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_comet_resume(opt) and not opt.evolve:
        ...
        opt.data, opt.cfg, opt.hyp, opt.weights, opt.project = \
            check_file(opt.data), check_file(opt.cfg), check_file(opt.hyp), str(opt.weights), str(opt.project)  # checks
