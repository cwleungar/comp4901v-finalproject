...
def train(model, device, config, epochs=5, batch_size=1, save_cp=True, log_step=20, img_scale=0.5):
   ...
   # writer.add_images('legend',
    ...
    for epoch in range(epochs):
        ...
        model.train()
        loop1=tqdm(enumerate(train_loader), total =len(train_loader), position=0, leave=True, ascii=True)

        for i, batch in loop1:
            global_step += 1
            epoch_step += 1
            images = batch[0]
            bboxes = batch[1]
        ...
        writer.add_scalar('val/mAP_0.5', stats[1], global_step)
        writer.add_scalar('val/mAP_0.5:0.95', stats[0], global_step)
...
def evaluate(model, data_loader, cfg, device, logger=None, **kwargs):
    ...
    loop2=tqdm(enumerate(data_loader), total =len(data_loader), position=0, leave=True, ascii=True)

    for i,(images, targets) in loop2:
