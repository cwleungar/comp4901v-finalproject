...
def convert_to_coco(input_file, output_dir, image_dir):
    ...
    with open(input_file, 'r') as f:
        ...
        for line_num, line in enumerate(lines):
            ...
            os.popen('cp '+os.path.join(image_dir, imgname)+' '+os.path.join(a, imgname))
            os.popen('cp '+os.path.join(output_dir, imgname.split('.')[0] + '.txt')+' '+os.path.join(b, imgname.split('.')[0] + '.txt'))
